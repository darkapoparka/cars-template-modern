...
const createBasehub = () => {
  ...
  return BASEHUB_TOKEN ? basehubClient({ token: BASEHUB_TOKEN }) : undefined;
};

type BasehubClient = ReturnType<typeof createBasehub>;

let client: BasehubClient;

const getBasehub = (): BasehubClient => {
  if (client) {
    return client;
  }

  client = createBasehub();
  return client;
};
