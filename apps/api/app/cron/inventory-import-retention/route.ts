import { env } from "@/env";
import { purgeInventoryImportRetention } from "@/lib/inventory-import-workers";
...
export const GET = async (request: Request): Promise<Response> => {
  ...
  try {
    const result = await purgeInventoryImportRetention(
    ...
    return serviceJson({ error: "inventory_import_retention_failed" }, 500);
  }
};
